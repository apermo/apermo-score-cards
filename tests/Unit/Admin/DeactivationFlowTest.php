...
namespace Apermo\ScoreCards\Tests\Unit\Admin;

// phpcs:disable SlevomatCodingStandard.Namespaces.AlphabeticallySortedUses.IncorrectlyOrderedUses -- The Apermo_Score_Cards\* imports get rewritten by setup.sh; final alphabetical position depends on the chosen namespace.
...
class DeactivationFlowTest extends TestCase {
	...
	public function test_register_wires_hooks(): void {
		Functions\stubs( [ 'plugin_basename' => 'apermo-score-cards/plugin.php' ] );

		( new DeactivationFlow() )->register();

		$this->assertTrue( has_filter( 'plugin_action_links_apermo-score-cards/plugin.php' ) > 0 );
		$this->assertTrue( has_filter( 'network_admin_plugin_action_links_apermo-score-cards/plugin.php' ) > 0 );
	...
	public function test_handle_destructive_dies_on_missing_capability(): void {
		Functions\stubs(
			[
				...
				'plugin_basename'              => 'apermo-score-cards/plugin.php',
	...
	public function test_handle_destructive_completes_full_flow(): void {
		...
		Functions\expect( 'delete_option' )
			->once()
			->with( 'apermo_score_cards_settings' );

		Functions\expect( 'deactivate_plugins' )
			->once()
			->with( 'apermo-score-cards/plugin.php', false, false );
	...
	public function test_handle_destructive_network_admin_path(): void {
		...
		Functions\expect( 'deactivate_plugins' )
			->once()
			->with( 'apermo-score-cards/plugin.php', false, true );
